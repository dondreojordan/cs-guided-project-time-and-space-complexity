...
def is_anagram(a, b):
    ...
    # contain the exact same number of each other

    # check if a and b have the same number of occurences
    # of each character as one another

    # store the number of occurences of a and b in their
    # own dicts
    ...
    for char, occurences in a_dict.items():
        if char not in b_dict:
            return False
        if b_dict[char] != occurences:
            return False
